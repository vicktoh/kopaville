...
import { Button, CheckIcon, CloseIcon, Flex, Heading, HStack, Icon, IconButton, Image, KeyboardAvoidingView, ScrollView, Text, VStack } from 'native-base';
import { useDispatch } from 'react-redux';
import { useAppSelector } from '../hooks/redux';
import { increaseQuantity, decreaseQuantity, removeItem, setCart} from '../reducers/cartSlice'
   ...
   return (
      <Flex width="100%">
         ...
         <Flex direction='row' alignItems="center" justifyContent="space-between" py={2}>
            <IconButton onPress={remove} icon={<Icon size="sm" color="red.300" as = {Feather} name = "trash-2" />} />
            <VStack alignItems="center">
               <Text fontSize="xs">Quantity</Text>
               <HStack space={3} alignItems="center">
               <IconButton onPress={decrease} disabled={quantity<=1} icon = {<Icon size="sm" as = {AntDesign} name = "minus" />} />
               <Text>{quantity}</Text>
               <IconButton onPress={increase}  icon = {<Icon size="sm" as = {AntDesign} name = "plus" />} />
            </HStack>
            </VStack>
...
export const CartScreen : FC<CartScreenProps> = ({ navigation })=> {
   const {cart, billing, auth} = useAppSelector(({cart, billing, auth})=> ({cart, billing, auth}));
   ...
   useEffect(()=>{
      function listenOnOrder(){
         if(transactionRef){
            const unsub = listenOnConfirmedOrder(transactionRef, (data: any) => {
               console.log(data);
   ...
   const checkout = ()=> {
      ...
      fetch(url, {
          ...
          } as any).then(async responseJson => {
             if(!cart) return;
   ...
   if(orderConfirmed){
      return(
         <Flex flex={1} safeArea justifyContent="center" alignItems="center" px={8}>
             <Icon my={2} size="xl" color="primary.400" as={AntDesign} name = "checkcircle" />
            <Heading textAlign="center" mb={1}>Your order Has been confirmed</Heading>
            <Text textAlign="center">Thank you for your purchase monitor the delivery of your products from your orders page</Text>
            <Button my = {5} variant="solid" onPress={()=> {navigation.goBack(); navigation.navigate("Orders")}} >View Order</Button>
         </Flex>
      )
   }
   ...
   return (
       ...
       >
           <ScrollView bg="white">
               <Flex flex={1} direction="column" safeArea position="relative" bg="white" >
                   <IconButton
