...
import { editJob, postJob } from '../services/jobServices';
...
const JobSchema = yup.object().shape({
    ...
    criteria: yup
        .array()
        .min(1, 'Must add at least on criteria')
        .max(5, 'Max of 5 criteria allowed'),
    bannerUrl: yup.string(),
});

export const AddJobForm: FC<AddJobFormProps> = ({ onCancel, job, mode }) => {
    const { auth, profile, systemInfo } = useAppSelector(
        ({ auth, profile, systemInfo }) => ({
            auth,
            profile,
            systemInfo,
        })
    ...
    const initialValue: Job & { formError?: string; bannerUri?: string } =
        job || {
            title: '',
            description: '',
            link: '',
                ...
                onSubmit={async (values, { setFieldValue, setSubmitting }) => {
                    try {
                        ...
                        if(mode === "add") {
                            ...
                            toast.show({title: 'Successfully Added post',});
                        }
                        if(mode === "edit") {
                            await editJob({...job, ...rest});
                            toast.show({title: 'Successfully Updated post',});
                        }
                        setSubmitting(false);
                        onCancel();
                    } catch (error) {
                        ...
                        toast.show({
                            ...
                            description:
                                err?.message ||
                                "Could not post job, make sure you're connected to the internet",
                        });
                    } finally {
                        setSubmitting(false);
                    }
                            ...
                            {values.bannerUrl || values.bannerUri ? (
                                ...
                                >
                                    <Heading
                                        ...
                                        color="white"
                                    >
                                        {`${mode === "add"? "Add": "Edit"} Job Opening`}
                                    </Heading>
                                </ImageBackground>
                            ) : (
                                <Heading ml={2} mt="auto" mb={2}>
                                    {`${mode === "add"? "Add": "Edit"} Job Opening`}
                                </Heading>
                            )}
                            <IconButton
                                ...
                                right={5}
                                top={0}
                                icon={<Icon size={5} as={Entypo} name="edit" />}
                                onPress={() =>
                                    pickImageFromGallery((uri) =>
                                        setFieldValue('bannerUri', uri)
                                    )
                                }
                            />
                        </Flex>
